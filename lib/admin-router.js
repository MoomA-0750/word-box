...
const { listFiles, safeWriteFile, isAllowedFile } = require('./files');
const { createBackup, listBackups, formatBytes } = require('./backup');
const Diff = require('diff');

const log = createLogger('admin');

// コンテンツディレクトリ（config から取得）
const CONTENT_DIRS = {
  posts: config.content.postsDir,
  topics: config.content.topicsDir,
  magazines: config.content.magazinesDir,
  dictionary: config.content.dictionaryDir,
};
...
// 記事一覧を取得（listFiles で micromatch ベースの絞り込み）
...
async function loadImageMetadata() {
  try {
    ...
    log.error('Failed to load image metadata:', err);
  }
  return {};
}
...
// 画像一覧を取得（micromatch でフィルタリング）
async function getImages() {
  await fs.ensureDir(IMAGES_DIR);
  const imageFiles = await listFiles(IMAGES_DIR, IMAGE_PATTERNS);
  const metadata = await loadImageMetadata();
  const images = [];

  // Promise.all で並列に stat 取得
  ...
  for (const { file, stats } of statsResults) {
...
// 画像をアップロード（isAllowedFile で micromatch ベースのチェック）
async function uploadImage(filename, data) {
  ...
  if (!isAllowedFile(filename.toLowerCase(), IMAGE_PATTERNS)) {
...
async function loadFileMetadata() {
  try {
    ...
    log.error('Failed to load file metadata:', err);
  }
  return {};
}
...
async function handleAdminRequest(req, res, urlPath, rebuildSearchIndex) {
  ...
  if (urlPath === '/admin/api/images' && method === 'POST') {
    try {
      ...
      log.error('Image upload error:', err.message);
      if (!res.headersSent) {
  ...
  // API: バックアップ作成（tar.gz）
  if (urlPath === '/admin/api/backup' && method === 'POST') {
    try {
      const result = await createBackup('./content');
      return sendJson(res, result, 201);
    } catch (err) {
      log.error('Backup failed:', err.message);
      return sendError(res, 'Backup failed: ' + err.message, 500);
    }
  }

  // API: バックアップ一覧
  if (urlPath === '/admin/api/backups' && method === 'GET') {
    const backups = await listBackups();
    return sendJson(res, backups.map(b => ({
      ...b,
  ...
  // API: バックアップダウンロード
  if (urlPath.match(/^\/admin\/api\/backups\/[^/]+$/) && method === 'GET') {
    const filename = decodeURIComponent(urlPath.split('/')[4]);
    // ディレクトリトラバーサル防止
    if (filename.includes('/') || filename.includes('\\') || filename.includes('..')) {
  ...
  // API: 記事の差分比較（diff）
  // POST /admin/api/diff  body: { oldText, newText }
  // 2つのテキストの差分をHTML形式で返す
  if (urlPath === '/admin/api/diff' && method === 'POST') {
    try {
      const body = await parseBody(req);
      ...
      return sendJson(res, { html: diffHtml, stats });
    } catch (err) {
      log.error('Diff error:', err.message);
      return sendError(res, 'Diff failed', 500);
    }
  }

  // API: 記事バージョン間の差分
  // GET /admin/api/diff/:type/:id  - 現在の保存内容とリクエストbodyの差分
  if (urlPath.match(/^\/admin\/api\/diff\/(posts|topics|magazines|dictionary)\/[^/]+$/) && method === 'POST') {
    try {
      const parts = urlPath.split('/');
      const type = parts[4];
      ...
      const changes = Diff.diffLines(oldBody, newBody);
      ...
      return sendJson(res, { html: diffHtml, stats, articleTitle: article.metadata.title || id });
    } catch (err) {
      log.error('Article diff error:', err.message);
      return sendError(res, 'Diff failed', 500);
    }
  }
