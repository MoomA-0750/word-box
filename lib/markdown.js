...
function parseMarkdown(text, allPosts = [], allMagazines = []) {
  let html = text;
...
<div class="code-block">
  ...
  <pre><code class="language-${language}">${escapedCode}</code></pre>
      ...
      calloutCodeBlocks.push(codeBlockHtml);
      return cbPlaceholder;
    });

    // 再帰的にMarkdownパースを適用（コードブロック以外の要素）
    let parsedContent = parseMarkdown(protectedContent, allPosts, allMagazines);
...
<div class="code-block">
  ...
  html = paragraphs.map(p => {
    ...
    // プレースホルダーのみの場合はスキップ
    if (p.match(/^___(CODE_BLOCK|BOOKMARK|ARTICLE|MAGAZINE|CALLOUT|TABLE|CONTENTS)_?\d*___$/) || p === '___CONTENTS___') {
      return p;
    }
    // プレースホルダーを含む複合段落の処理
    if (p.match(/___(CODE_BLOCK|BOOKMARK|ARTICLE|MAGAZINE|CALLOUT|TABLE|CONTENTS)_?\d*___/)) {
      // プレースホルダーと通常テキストを分離して処理
      const parts = p.split(/(___(CODE_BLOCK|BOOKMARK|ARTICLE|MAGAZINE|CALLOUT|TABLE|CONTENTS)_?\d*___)/);
      return parts.map(part => {
        if (part.match(/^___(CODE_BLOCK|BOOKMARK|ARTICLE|MAGAZINE|CALLOUT|TABLE|CONTENTS)_?\d*___$/)) {
          return part;
        }
        if (part.match(/^(CODE_BLOCK|BOOKMARK|ARTICLE|MAGAZINE|CALLOUT|TABLE|CONTENTS)$/)) {
...
function extractPlainText(text) {
  ...
  // 独自ブロックを除去 (:::bookmark, :::article, :::magazine, :::contents)
  plain = plain.replace(/:::(bookmark|article|magazine|contents)\n[\s\S]*?:::/g, '');
