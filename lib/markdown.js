...
function createTableOfContents(headings) {
  ...
  let tocHtml = '<nav class="toc"><div class="toc-title">目次</div><ul class="list-unstyled mb-0">';
...
function createBookmarkCard(content) {
  ...
  return `
<a href="${escapeHtml(url)}" class="d-flex align-items-center gap-3 p-3 border rounded text-decoration-none mb-3 bookmark-card" target="_blank" rel="noopener noreferrer">
  <div class="flex-shrink-0 lh-1" style="font-size:1.5rem">${icon}</div>
...
function createMagazineCard(slug, allMagazines, allPosts) {
  ...
  if (!magazine) {
    // マガジンが見つからない場合
    return `
<div class="d-flex align-items-center gap-3 p-3 border rounded mb-3 text-muted">
  <span>❌</span>
  <span>マガジンが見つかりません: ${escapeHtml(slug)}</span>
</div>`;
  }
  ...
  return `
<a href="/magazines/${escapeHtml(magazine.slug)}" class="d-flex align-items-center gap-3 p-3 border rounded text-decoration-none mb-3 magazine-card-embed">
  <div class="flex-shrink-0 lh-1" style="font-size:2rem">${escapeHtml(magazine.emoji)}</div>
  <div class="flex-grow-1">
    <div class="fw-semibold">${escapeHtml(magazine.title)}</div>
    <div class="text-muted small">${escapeHtml(magazine.description)}</div>
    <span class="badge bg-secondary mt-1">${articleCount}件の記事</span>
  </div>
  <span class="text-muted flex-shrink-0">→</span>
</a>`;
}
...
<div class="d-flex align-items-center gap-3 p-3 border rounded mb-3 text-muted">
  <span>❌</span>
  <span>記事が見つかりません: ${escapeHtml(slug)}</span>
</div>`;
  }

  // タグのHTML生成
  const tagsHtml = post.tags.length > 0
    ? post.tags.map(tag => `<span class="badge rounded-pill bg-secondary">${escapeHtml(tag)}</span>`).join('')
    : '';

  // quicklookがあればそれを、なければ日付を表示
  const subtitleHtml = post.quicklook
    ? `<span class="text-muted small">${escapeHtml(post.quicklook)}</span>`
    : `<time class="text-muted small">${escapeHtml(post.date)}</time>`;

  return `
<a href="/posts/${escapeHtml(post.slug)}" class="d-flex align-items-center gap-3 p-3 border rounded text-decoration-none mb-3 article-card">
  <div class="flex-shrink-0 lh-1" style="font-size:2rem">${escapeHtml(post.emoji)}</div>
  <div class="flex-grow-1">
...
<div class="d-flex align-items-center gap-3 p-3 border rounded mb-3 text-muted">
  <span>❌</span>
  <span>辞書項目が見つかりません: ${escapeHtml(slug)}</span>
</div>`;
  }

  return `
<a href="/dictionary/${escapeHtml(entry.slug)}" class="card text-decoration-none mb-3 dictionary-card-embed">
  <div class="card-header d-flex align-items-center gap-2 py-1 px-3 small text-muted">
    <span>📖</span><span>辞書</span>
  </div>
  <div class="card-body d-flex align-items-center gap-3">
    <div class="flex-shrink-0 lh-1" style="font-size:2rem">${escapeHtml(entry.emoji)}</div>
    <div class="flex-grow-1">
